...
- **数据库**: PostgreSQL, Redis - 用于存储用户自选股、历史行情及缓存
- **IDL**: Thrift
...
- **微服务架构**:
  ...
  - **AI 服务 (AI Service)**: 基于 Kitex 的 RPC 服务，集成多种 LLM 模型（OpenAI, Zhipu, Qwen, Doubao, DeepSeek），负责行情分析、预测及图片识别。
  - **股票服务 (Stock Service)**: 基于 Kitex 的 RPC 服务，负责行情数据（新浪/东方财富）、龙虎榜、涨停池及用户数据管理。
- **通信**: 客户端与网关之间使用 HTTP/JSON；网关与服务之间使用 Thrift/Kitex。
- **数据流**: 移动端 -> 网关 -> 服务 -> (外部 API/LLM/数据库)。
...
## 领域背景
- **市场**: 中国 A 股（上海/深圳）。
- **关键实体**: 股票（代码、名称、价格）、板块、大盘指数、龙虎榜席位、涨停原因。
- **功能**: 实时价格监控、每日市场总结、基于 AI 的价格预测、龙虎榜分析、涨停池监控、图片识别加自选。

## 重要约束
- **无内部依赖**: 严格使用公开的开源库。
- **隐私**: 仅通过轻量级用户 ID 关联数据；优先使用本地存储保存用户偏好。
- **语言**: 应用内容必须为中文。
