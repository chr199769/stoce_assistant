...
func (m *LangfuseManager) GetPrompt(ctx context.Context, name string, version *int) (string, error) {
	...
	path := fmt.Sprintf("/api/public/prompts/%s", name)
	if version != nil {
...
func (m *LangfuseManager) Span(ctx context.Context, traceID string, parentObservationID *string, name string, input interface{}, output interface{}, startTime, endTime time.Time) error {
	_, err := m.SpanWithID(ctx, traceID, parentObservationID, name, input, output, startTime, endTime)
	return err
}

func (m *LangfuseManager) SpanWithID(ctx context.Context, traceID string, parentObservationID *string, name string, input interface{}, output interface{}, startTime, endTime time.Time) (string, error) {
	if m == nil {
		return "", fmt.Errorf("langfuse not initialized")
	}
